var cicdUpdateSetUtils = Class.create();
cicdUpdateSetUtils.prototype = {
    initialize: function() {
		this.updateSetBatchingURL = "https://docs.servicenow.com/bundle/paris-application-development/page/build/system-update-sets/hier-update-sets/concept/us-hier-overview.html";
		...
		this.scriptSysID = "18b33dd7db24a81c4f73498039961997";
		this.scriptAPIName = "global.cicdUpdateSetUtils";
    },
	...
	compileConfirmationMessage: function(isList) {
		...
		var errorMessages = this.clientSession.getClientData("errorMessages") + "";
		if (!isList && errorMessages != "null" && errorMessages.length > 0) {
			errorMessages = errorMessages.split(",");
    ...
    checkTable: function(tableRec, tableName) {
		...
		switch (tableName) {
			...
			/********************* Connected Operations Tables *****************************/
			case "sn_smart_ops_core_digital_twin_definition":
				this._addCODigitalTwinDefinition(tableRec, tableName);
                continueProcessing = false;
				break;
	...
	*/
    ...
    _addCatItem: function(tableRec, tableName,skipItems) {
        this.saveRecord(tableRec);

        var catItemID = tableRec.sys_id.toString();

		//Skip variableset if passed in skipItems
		var addVariableSet = true;
		if(skipItems && skipItems.length>0 && skipItems.indexOf('variable_set')>-1){
			addVariableSet=false;
		}
		var variableSetList = [];
		if(addVariableSet){
	...
	_addReport: function(sysReport, tableName) {
		...
		if (reportSource.get(sysReport.getValue("report_source"))) {
	...
	/********************* Begin Connected Operations Functions *********************/
	_addCODigitalTwinDefinition: function(dtDefinition, tableName) {
		this.saveRecord(dtDefinition);
		var dtDefinitionID = dtDefinition.getValue("sys_id");
		
		var productModel = new GlideRecord("cmdb_model");
		productModel.addQuery("digital_twin_definition", dtDefinitionID);
		productModel.query();
		while (productModel.next()) {
			this.saveRecord(productModel);
		}
		
		var operationalAttributeList = [];
		var operationalAttribute = new GlideRecord("sn_smart_ops_core_operational_attribute");
		operationalAttribute.addQuery("digital_twin_definition", dtDefinitionID);
		operationalAttribute.query();
		while (operationalAttribute.next()) {
			this.saveRecord(operationalAttribute);
			operationalAttributeList.push(operationalAttribute.getValue("sys_id"));
		}
		
		var connectedAttributeList = [];
		var connectedAttribute = new GlideRecord("sn_iot_hub_connected_attribute");
		connectedAttribute.addQuery("operational_attribute", operationalAttributeList);
		connectedAttribute.query();
		while (connectedAttribute.next()) {
			this.saveRecord(connectedAttribute);
			connectedAttributeList.push(connectedAttribute.getValue("sys_id"));
		}
		
		var payloadMapping = new GlideRecord("sn_iot_hub_payload_map");
		payloadMapping.addQuery("connected_attribute", connectedAttributeList);
		payloadMapping.query();
		while (payloadMapping.next()) {
			this.saveRecord(payloadMapping);
		}
		
		var workspaceRenderer = new GlideRecord("sys_aw_renderer");
		workspaceRenderer.addQuery("table", dtDefinition.getValue("asset_table"));
		workspaceRenderer.query();
		while (workspaceRenderer.next()) {
			this.saveRecord(workspaceRenderer);
		}
		
		var eventType = new GlideRecord("sn_iot_hub_event_type");
		eventType.addQuery("digital_twin_definition", dtDefinitionID);
		eventType.query();
		while (eventType.next()) {
			this.saveRecord(eventType);
		}
		
		var iotRuleList = [];
		var iotRuleVersion = new GlideRecord("sn_iot_hub_iot_rule_version");
		iotRuleVersion.addQuery("digital_twin_definition", dtDefinitionID);
		iotRuleVersion.query();
		while (iotRuleVersion.next()) {
			this.saveRecord(iotRuleVersion);
			var iotRuleID = iotRuleVersion.getValue("iot_rule");
			if (iotRuleList.indexOf(iotRuleID) == -1) {
				iotRuleList.push(iotRuleID);
			}
			
			var flowID = iotRuleVersion.getValue("flow");
			if (!gs.nil(flowID)) {
				this._addFlow(flowID);
			}
		}
		
		var iotRule = new GlideRecord("sn_iot_hub_iot_rule");
		iotRule.addQuery("sys_id", iotRuleList);
		iotRule.query();
		while (iotRule.next()) {
			this.saveRecord(iotRule);
		}
	},
	
	/********************* End Connected Operations Functions *********************/

    type: 'cicdUpdateSetUtils'
};